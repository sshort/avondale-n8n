...
- `sync-raw-tables-to-cloud.json`: the local source-of-truth sync workflow. It full-syncs `raw_contacts` and `raw_members`, upserts `member_signups` and `signup_batches` into cloud by default, and also syncs `n8n.workflow_entity` and `n8n.execution_entity` so the cloud Metabase execution cards reflect the local server.
...
- `send-no-address-batch-emails.json`: the local workflow that emails batch recipients who have email but do not have a valid postal address. It uses the `shoe_tag_pigeon_hole` template and supports `production` and `test` delivery modes.
