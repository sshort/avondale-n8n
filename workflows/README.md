...
- `clubspark-full-refresh.json`: the parent orchestration workflow for local n8n. It runs the contacts export, members export, main contacts export, and cloud sync workflows in sequence, reusing the same ClubSpark browser session between export steps, and ends in explicit success or error summary nodes.
