...
const sql = `WITH monthly AS (
  SELECT
    ...
    EXTRACT(MONTH FROM signup_date) AS cal_mo,
    COUNT(*) AS cnt
  FROM member_signups
...
pivoted AS (
  SELECT
    CASE WHEN cal_mo >= 3 THEN cal_mo - 2 ELSE cal_mo + 10 END AS mo,
    TO_CHAR(
      MAKE_DATE(
        CASE WHEN cal_mo >= 3 THEN 2000 ELSE 2001 END,
        cal_mo::integer, 1
      ), 'Month'
    ) AS month_name,
...
const updated = await api('PUT', `/api/card/${CARD_ID}`, cardPayload);
console.log(`Card ${CARD_ID} updated successfully.`);
console.log(`View at: http://${HOST}:${PORT}/question/${CARD_ID}`);
