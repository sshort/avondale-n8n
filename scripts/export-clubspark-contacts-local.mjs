...
if (providedCookieHeader) {
  const cookieCount = await applyCookieHeaderToContext(context, contactsUrl, providedCookieHeader);
  if (cookieCount) {
    console.error(`[clubspark-export] Applied ${cookieCount} cookies from reusable auth session`);
  }
}
...
async function isAuthenticatedContactsPage() {
  const snapshot = await textSnapshot();
  return (
    /\/Admin\/Contacts(?:[/?#]|$)/i.test(snapshot.url)
    && /Contact Options|Reset filters|Export CSV|contacts-table/i.test(snapshot.bodyText)
...
try {
  ...
  const reusedAuthenticatedSession =
    Boolean(providedCookieHeader) && await isAuthenticatedContactsPage();
