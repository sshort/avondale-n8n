# Team Match Review

This file records the current non-trivial name matching used by [generate_team_contact_lists.py](./generate_team_contact_lists.py).

## Remaining No Match Names


## No Contact Information (Matched but No Contact Details)


## Not Signed Up (Contact Only, No Current Membership)

- `Alistair Jones` - contact-only match - no current membership. Teams: A Squad, Reserves
- `Ben Fowler` - contact-only match - no current membership. Teams: E Squad
- `David Jones` - contact-only match - no current membership. Teams: A Squad
- `Jacob Diskin` - contact-only match - no current membership. Teams: B Squad
- `Kate Liggett` - contact-only match - no current membership. Teams: C Squad

## No Consent Contact Details

- `Alec Mach` - self contact details do not permit onward sharing. Teams: B Squad
- `Alek Katon` - self contact details do not permit onward sharing. Teams: D Squad
- `Alessandro Barcello` - self and parent contact details do not permit onward sharing. Teams: E Squad
- `Alex katon` - self contact details do not permit onward sharing. Teams: B Team, D Squad
- `Alex Mach` - self contact details do not permit onward sharing. Teams: C Squad
- `Alistair Jones` - self contact details do not permit onward sharing. Teams: A Squad, Reserves
- `Amy Palmer` - parent contact details do not permit onward sharing. Teams: A Squad
- `Andy Mc Elligot` - self contact details do not permit onward sharing. Teams: B Squad, Reserves
- `Antonias Nicolaidis` - parent contact details do not permit onward sharing. Teams: D Squad
- `Antonis Nicolaidis` - parent contact details do not permit onward sharing. Teams: D Squad
- `Ben Hammond Duncan` - self contact details do not permit onward sharing. Teams: D Squad, E Squad
- `Charlene Jacobs` - self contact details do not permit onward sharing. Teams: B Squad
- `Edward Long` - self contact details do not permit onward sharing. Teams: Reserves
- `Fran Jones` - self contact details do not permit onward sharing. Teams: C Squad, D Squad
- `Harry McIntyre` - self contact details do not permit onward sharing. Teams: D Squad
- `Helen Eyres` - self contact details do not permit onward sharing. Teams: E Squad
- `James Cholerton` - self contact details do not permit onward sharing. Teams: Reserves
- `Jessica Palmer` - self contact details do not permit onward sharing. Teams: C Team, D Squad, E Squad
- `Jo Baines` - self contact details do not permit onward sharing. Teams: E Squad
- `Joel Villamayor` - self contact details do not permit onward sharing. Teams: D Squad, E Squad
- `Katherine Rogers` - self contact details do not permit onward sharing. Teams: B Team, C Squad
- `Leila Warwick` - self contact details do not permit onward sharing. Teams: A Squad
- `Lindsey Evans` - self contact details do not permit onward sharing. Teams: E Squad
- `Liz Gruenstern` - self contact details do not permit onward sharing. Teams: E Squad
- `Liz Gruernstern` - self contact details do not permit onward sharing. Teams: D Squad
- `Liz Long` - self contact details do not permit onward sharing. Teams: B Squad, Reserves
- `Michael Palmer` - self contact details do not permit onward sharing. Teams: B Squad
- `Mo Clackett` - self contact details do not permit onward sharing. Teams: Reserves
- `Nigel Fisher` - self contact details do not permit onward sharing. Teams: D Squad, E Squad
- `Oliver Casselton` - self contact details do not permit onward sharing. Teams: B Squad
- `Paul Kemsley` - self contact details do not permit onward sharing. Teams: A Squad
- `Rob Bozier` - self contact details do not permit onward sharing. Teams: B Squad
- `Roni Asp` - self contact details do not permit onward sharing. Teams: B Squad
- `Sam Rush` - self contact details do not permit onward sharing. Teams: A Squad, B Squad
- `Santu Asp` - self contact details do not permit onward sharing. Teams: B Squad
- `Sue white` - self contact details do not permit onward sharing. Teams: A Team, B Squad, E Squad
- `Susie Cooke` - self contact details do not permit onward sharing. Teams: D Squad
- `Tom Van Klaveren` - self contact details do not permit onward sharing. Teams: B Squad
- `Tom Van Klavern` - self contact details do not permit onward sharing. Teams: B Squad

## Explicit Full-Name Overrides

- `Carol Ann Harrison` -> `Carol-Anne Harrison` - explicit full-name override from name_overrides.csv. Teams: D Squad
- `CAROLE -ANN HARRISON` -> `Carol-Anne Harrison` - explicit full-name override from name_overrides.csv. Teams: C Team
- `Jane Dow` -> `JANE Timmis` - explicit full-name override from name_overrides.csv. Teams: A Squad, A Team
- `Jane Woods` -> `Alison Woods` - explicit full-name override from name_overrides.csv. Teams: Reserves
- `Jax S B` -> `Jacqueline Sinclair-Brown` - explicit full-name override from name_overrides.csv. Teams: D Squad, E Squad
- `JAX S BROWN` -> `Jacqueline Sinclair-Brown` - explicit full-name override from name_overrides.csv. Teams: C Team
- `Tom Van Klavern` -> `Thomas Van Klaveren` - explicit full-name override from name_overrides.csv. Teams: B Squad

## Cross-Family Contact Matches

- `Michael Palmer` -> `` - contact details from family member with same surname. Teams: B Squad

## Short-Name / Nickname Matches

- `Alec Mach` -> `Aleksander Mach` - nickname mapping from nicknames.csv. Teams: B Squad
- `Alex Mach` -> `Aleksander Mach` - nickname mapping from nicknames.csv. Teams: C Squad
- `Andy Roxburgh` -> `Andrew Roxburgh` - nickname mapping from nicknames.csv. Teams: B Squad
- `Becky Cockerill` -> `Rebecca Cockerill` - nickname mapping from nicknames.csv. Teams: Reserves
- `Jacquie Conway` -> `Jacqueline Conway` - nickname mapping from nicknames.csv. Teams: A Team, B Squad
- `Jon Clackett` -> `Jonathan Clackett` - nickname mapping from nicknames.csv. Teams: C Squad, Reserves
- `Lynne McKenzie` -> `Lynn Mckenzie` - nickname mapping from nicknames.csv. Teams: C Squad
- `Matt Napper` -> `Matthew Napper` - nickname mapping from nicknames.csv. Teams: E Squad, Reserves
- `Mo Clackett` -> `Maureen Clackett` - nickname mapping from nicknames.csv. Teams: Reserves
- `PETE SMITH` -> `Peter Smith` - nickname mapping from nicknames.csv. Teams: B Team
- `Sam Rush` -> `Samantha Rush` - nickname mapping from nicknames.csv. Teams: A Squad, B Squad
- `Sue white` -> `Susan White` - nickname mapping from nicknames.csv. Teams: A Team, B Squad, E Squad
- `Tom Van Klaveren` -> `Thomas Van Klaveren` - nickname mapping from nicknames.csv. Teams: B Squad
- `Val Rowlands` -> `Valerie Rowlands` - nickname mapping from nicknames.csv. Teams: Reserves

## Fuzzy Matches

- `Alek Katon` -> `Alex Katon` - fuzzy name match. Teams: D Squad
- `Alessandro Barcello` -> `Alessandro Barcella` - fuzzy name match. Teams: E Squad
- `Alison Jamieson` -> `Alison Jameson` - fuzzy name match. Teams: C Squad
- `Andy Mc Elligot` -> `Andy McElligott` - fuzzy name match. Teams: B Squad, Reserves
- `Antonias Nicolaidis` -> `Antonios Nicolaidis` - fuzzy name match. Teams: D Squad
- `Antonis Nicolaidis` -> `Antonios Nicolaidis` - fuzzy name match. Teams: D Squad
- `Graeme Hutcheson` -> `Graeme Hutchison` - fuzzy name match. Teams: D Squad
- `Helen Eyres` -> `Helen Eyers` - fuzzy name match. Teams: E Squad
- `Jill Penton` -> `Jillian Penton` - fuzzy name match. Teams: B Team, Reserves
- `Lindsey Evans` -> `Lindsay Evans` - fuzzy name match. Teams: E Squad
- `Liz Gruernstern` -> `Liz Gruenstern` - fuzzy name match. Teams: D Squad
- `Maew Tatum` -> `Maew Tatam` - fuzzy name match. Teams: D Squad
- `Paul Tippens` -> `Paul Tippins` - fuzzy name match. Teams: D Squad, Reserves
- `Santu Asp` -> `Santtu Asp` - fuzzy name match. Teams: B Squad
- `Trish Harris` -> `Trish Harriss` - fuzzy name match. Teams: C Squad, C Team
